...
function getSpanningConfig(len,columnLen){
    ...
    let configs=[]
    for(let i=0;i<len;i++){
        configs.push( {s:{r:i*3+start,c:0},e:{r:i*3+start+2,c:0}})
        configs.push( {s:{r:i*3+start,c:1},e:{r:i*3+start+2,c:1}})
        configs.push( {s:{r:i*3+start,c:3},e:{r:i*3+start+2,c:3}})
        configs.push( {s:{r:i*3+start,c:columnLen-1},e:{r:i*3+start+2,c:columnLen-1}})
    }
    return configs
}
// 设置单元格宽度
...
const handleAddress = (item, arr) => {
    ...
    let alias=new Map([['世茂天城','公司'],['世茂','公司']])
...
function handleRow(row, index = 0) {
    ...
    return [
        ...
        ['', '', '下午','',...row.logs.map((item) => handleAddress(item[1], item))],
        ['', '', '晚上','', ...row.logs.map((item) => handleAddress(item[2], item))]
    ]
}
...
function saveResultXlsx(outputPath, data) {
    ...
    const row2= ['序号','姓名','时间','出勤天数',...new Array(daysInMonth).fill(0).map((_,i)=>i+1),'备注']
    const sheetOptions = {
    ...
    fs.writeFileSync('record-excels/result.xlsx', buffer)
}
module.exports = {
    saveResultXlsx
}
